...
const index = () => {
  ...
  const editUser = (id: string) => {
    axios
      .get(`https://belanjalagiyuk.shop/users/${id}`, {
        headers: {
  ...
  const getUser = () => {
    axios
      .get(
        `https://belanjalagiyuk.shop/users?page=${page}&itemsPerPage=${itempage}`,
        {
          headers: {
  ...
  const HandleSubmitUser = () => {
    axios
      .post(
        "https://belanjalagiyuk.shop/users",
        {
          name: fullname,
          email: email,
          password: password,
  ...
  const handleDeleteUser = (id: number) => {
    axios
      .delete(`https://belanjalagiyuk.shop/users/${id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
  ...
  const handleEditUser = () => {
    axios
      .put(
        `https://belanjalagiyuk.shop/users/${editData}`,
        {
          name: editfullname,
          email: editemail,
          team: editteam,
        ...
        <div>
          <Modalclass isOpen={popup} onClose={() => setPopup(false)}>
            <div className="w-[50vw] h-[90vh] flex flex-col">
              ...
              <div>
                <Button label="Save" onClick={() => HandleSubmitUser()} />
              </div>
            </div>
          </Modalclass>
        </div>
